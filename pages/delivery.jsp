...
//讀取已存在Client端的Cookie
		...
		double delivery=0;
		String order_delivery=request.getParameter("order-delivery");
		String order_address=request.getParameter("order-address");
		if(order_delivery!=null){
			delivery=60;
		}
		else if(order_address!=null){
			delivery=100;
		}
  ...
  <div class="container">
    <div class="box col-sm-10" style="margin:45px auto;">
      ...
      <form action="back_add.jsp" method="POST">
        ...
        <label>收件人姓名：</label><label style="color: red;">*</label><input class="form-control" name="order-name" placeholder="請輸入姓名"
          required><br />
        <label>連絡電話：</label><label style="color: red;">*</label><input class="form-control" name="order-phone" placeholder="請輸入連絡電話"
          required><br />

        <label>寄送方式：</label><label style="color: red;">*</label><select class="form-control" name="order-delivery" required>
          <option disabled selected hidden>請選擇寄送方式</option>
          <option value="7-11">7-11</option>
          <option value="全家">全家</option>
          <option value="宅配">宅配</option>
        </select><br />
        <label>如選擇超商，請輸入超商門市：</label><input class="form-control" name="order-store" placeholder="請輸入門市名稱"><br /><br />
        <label>如選擇宅配，請輸入地址：</label><input class="form-control" name="order-address" placeholder="請輸入地址"><br /><br />
        <label>支付方式：</label><label style="color: red;">*</label><select class="form-control" name="order-pay" required>
          <option disabled selected hidden>請選擇支付方式</option>
        ...
        <div class="text-center">
          <input type="submit" value="送出訂單" class="btn btn-info">
        </div>
